...
@Reducer
struct NewChallengeFormFeature {
    @ObservableState
    struct State {
...
struct NewChallengeFormView: View {
    @Bindable var store: StoreOf<NewChallengeFormFeature>
...
//                }
            ...
            case .singleAnswer(var content):
                TextField("Answer", text: Binding(
                    get: { content.answer },
                    set: {
                        content.answer = $0
                        store.send(.updateChallengeContent(.singleAnswer(content)))
                    }
                ))
            case .noChoice:
                EmptyView()
...
#Preview {
    NewChallengeFormView(
        store: Store(initialState: NewChallengeFormFeature.State()) {
            NewChallengeFormFeature()
        }
    )
}
