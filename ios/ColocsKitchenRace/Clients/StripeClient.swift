...
@DependencyClient
struct StripeClient {
    var reserveAndCreatePayment: @Sendable (
        _ gameId: String,
        _ cohouseId: String,
        _ amountCents: Int,
...
extension StripeClient: DependencyKey {
    ...
    static let liveValue = Self(
        reserveAndCreatePayment: { gameId, cohouseId, amountCents, participantCount, attendingUserIds, averageAge, cohouseType in
            Logger.paymentLog.info("Reserving spot and creating payment intent for game \(gameId), cohouse \(cohouseId), amount \(amountCents) cents, \(participantCount) participants")

            let functions = Functions.functions(region: "europe-west1")
            let callable = functions.httpsCallable("reserveAndCreatePayment")
            ...
            guard let dict = result.data as? [String: Any],
                ...
                Logger.paymentLog.error("Invalid response from reserveAndCreatePayment")
                throw StripeError.invalidResponse
            }

            Logger.paymentLog.info("Spot reserved and payment intent created: \(paymentIntentId)")
    ...
    static let previewValue = Self(
        reserveAndCreatePayment: { _, _, _, _, _, _, _ in
            PaymentIntentResult(
