    ...
    static let liveValue = Self(
        ...
        listenToNews: {
            AsyncStream { continuation in
                // Demo mode: yield mock data immediately without creating a Firestore listener
                if DemoMode.isActive {
                    @Shared(.news) var sharedNews
                    $sharedNews.withLock { $0 = DemoMode.demoNews }
                    continuation.yield(DemoMode.demoNews)
                    return
                }
